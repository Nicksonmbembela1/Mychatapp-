...
export default function Home() {
  ...
  const [totalUsers, setTotalUsers] = useState(0); // 1. Imeongezwa
  ...
  useEffect(() => {
    ...
    if(!name) {
      ...
      loadChats();
      getTotalUsers(); // 1. Imeongezwa
    }
  }, [router]);

  const getTotalUsers = async () => { // 1. Function mpya
  ...
  const handleSearch = async () => {
    ...
    const { data, error: supaError } = await supabase.from('users').select('username').eq('username', usernameToAdd).single(); // 3. Imeongezwa error
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
      <div className="bg-blue-500 text-white p-4 shadow-md">
        ...
        <p className="text-sm opacity-80">@{myName} • Watumiaji: {totalUsers}</p> {/* 1. Imebadilika */}
      </div>

      <div className="p-3 bg-white border-b">
        <div className="flex gap-2">
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} placeholder="Andika @username kuongeza mtu..." className="flex-1 p-3 border rounded-full outline-none"/>
          <button onClick={handleSearch} disabled={loading} className="bg-blue-500 text-white px-5 rounded-full font-bold disabled:bg-gray-400">{loading? '...' : '+'}</button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {chatList.length === 1 && ( // 2. Imeongezwa
